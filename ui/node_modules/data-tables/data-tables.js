  ...
  .directive('dTable', function($timeout){
   ...
   //          dData : "&"
      ...
      $scope.ascending = "ascending";
        ...
        if($attrs.dAnimate == 'false') $scope.isAnimate = false;

        console.log("Animate:" + $scope.isAnimate)

            if($attrs.noPagination != undefined) {
                $scope.isNoPagination = true;
            }
            if($attrs.dMaxRows == undefined || $attrs.dMaxRows == '' ) $scope.isNoPagination = true;
            ...
            $scope.tbody = $element.find('tbody'); 
		 	...
		 	$scope.sortBy = function(e, field) {

        var a = 'sort_' + field;
        if($scope[a] == "ascending") $scope[a] = "descending";
        else $scope[a] = "ascending"
       ...
       $scope.prePage = function() {
                 if($scope.isLastPageLinkClicked) {
                    $scope.filterPage($scope.$currentPage - 3);
            ...
            $scope.animate = function() {
		 	...
		 	$scope.doPagination = function(cur, n) {
		 		if($scope.isNoPagination) return;
		 		 var page_no_count;
		...
		// require: 'ngModel', // Array = multiple requires, ? = optional, ^ = check parent elements
		 ...
		 template: function(iElm, attrs) {
            ...
            repeatString = repeatItem +" in filtered = ( filtered_search = (" + repeatData + customFilter + " | filter:$search | orderBy:$field:$reverse) | startFrom:$currentPage*$pageSize | limitTo:$pageSize | orderBy:'+':true ) track by $index";
            ...
            header = header[0].outerHTML;
      			...
      			header = header.replace(/d-sort=["'](.*?)['"]/gi, "ng-class='{{sort_$1}}' ng-click=\"sortBy($event, '$1')\"");
            ...
            var paginationHtml = `<div class="pager" ng-if="!isNoPagination && dataExists">
            ...
            var noRecordsHtml = `<div ng-if="isTablesDataLoaded" class="no-rec"><md-progress-circular ng-disabled="!isTablesDataLoaded" class="md-hue-2" md-diameter="20px"></md-progress-circular> <span style="float:left;position:relative;top:-17px;left:30px;">Loading data</span></div>
        ...
        var default_style = iAttrs.dDefaultStyle;
        var cu_style = iAttrs.dClass;
        var final_style;
        if(default_style == "false") final_style = cu_style; 
        else final_style = "d-table " + cu_style;
        iElm.addClass(final_style)
                    ...
                    $scope.$watch('filtered', function (newValue, oldValue, scope) {
                       ...
                       if($scope.filtered) {
                            ...
                            if(!$scope.initCall) {
                         	...
                         	$scope.isTablesDataLoaded = false;
                       	}
                       	else {
                           $scope.dataExists = false;
                           $scope.isTablesDataLoaded = true;
                       	}
